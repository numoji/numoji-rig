...
class UiSpacingConfigure(bpy.types.Operator):
    ...
    def draw(self, context):
        ...
        if armature:
            ...
            column.prop(
                armature,
                "ui_horizontal_spacing",
                text="Horizontal Spacing",
                slider=True,
            )
            column.prop(
                armature, "ui_vertical_spacing", text="Vertical Spacing", slider=True
            )
        else:
            self.layout.label(
                text="Select a roblox character rig to configure panel spacing",
                icon="INFO",
            )
...
class UiMainPanel(bpy.types.Panel):
    ...
    def draw_header(self, context):
        ...
        options.operator(
            "roblox_rig_ui.spacing_configure", text="", icon="PREFERENCES", emboss=False
        )
